...
export function TabBar({ tabs, active, onSelect, onClose }: Props): React.ReactElement | null {
  ...
  return (
    <div className="flex items-stretch h-9 bg-app border-b border-border overflow-x-auto shrink-0">
      {tabs.map((t) => {
        ...
        return (
          <div
            ...
            className={`group flex items-center gap-2 pl-3 pr-1.5 border-r border-border text-[12.5px] cursor-pointer whitespace-nowrap transition-colors ${
              isActive ? 'bg-surface text-fg' : 'text-fg/55 hover:bg-surface/60'
            }`}
          >
            ...
            <button
              ...
              className="w-4 h-4 grid place-items-center rounded text-fg/35 hover:text-fg hover:bg-border opacity-0 group-hover:opacity-100 focus:opacity-100 transition"
            >
