# Chuckle + Superpowers integration

Chuckle gates implementation behind human approval of spec/plan documents.
Integration is **opt-in**: with no `chuckle` MCP server and no `chuckle-gate`
hook in `.claude/settings.json`, your skills behave exactly as before.

## Setup

1. `npm install -g @chuckle/mcp-server @chuckle/superpowers-hook`
2. Create a vault in the Chuckle desktop app and note its absolute path.
3. Add both the MCP server and the hook to your project's
   `.claude/settings.json` (see `docs/chuckle-settings-example.json`).

## How it works

- **The hook (`chuckle-gate`)** is the hard gate. On every `Write`/`Edit`/
...
- **The publish convention** is how Claude tells Chuckle which feature is
